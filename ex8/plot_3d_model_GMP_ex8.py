...
data = gp.create_geomodel(
    ...
    extent=[246-1, 1444+1, 
            -2055-1, -167+1, 100, 550],
    refinement=6, 
    importer_helper=gp.data.ImporterHelper(
        path_to_orientations=repo_path + task + r"\orientations_csv.csv",
        path_to_surface_points=repo_path + task + r"\surface_points_csv_new.csv"
    )
)
...
gp.map_stack_to_surfaces(
    ...
    mapping_object={"Fault_Series": ('fault'),
                    "Strat_Series_1": ('S_2', 'S_1'), 
                    "Strat_Series_2": ('P', 'M', 'T'), }
)

gp.set_is_fault(data, ['Fault_Series'])


gp.set_topography_from_random(
    grid=data.grid,
    fractal_dimension=1.3,
    d_z=np.array([350, 550]),
...
with open(repo_path + task + r'\GMP_ex8_geomodel.pkl', 'wb') as f:
