...
task = r'\ex9\surf_points_orientations'
...
df_o = pd.read_csv(repo_path + task + r'\orientations.csv', sep=';')
# df_o['formation'] = df_o['formation'].astype(str)
df_s = pd.read_csv(repo_path + task + r"\surface_points.csv", sep=';')
# df_s['formation'] = df_s['formation'].astype(str)
df_o.to_csv(repo_path + task + r"\orientations_comma.csv", sep=',', index = False)
df_s.to_csv(repo_path + task + r"\surface_points_comma.csv", index = False)




#%%
df_o = pd.read_csv(repo_path + task + r'\orientations_comma.csv', sep=',')
print(df_o.head)

data = gp.create_geomodel(
    project_name='GMP_ex9_model',
    ...
    importer_helper=gp.data.ImporterHelper(
        path_to_orientations=repo_path + task + r"\orientations_comma.csv",
        path_to_surface_points=repo_path + task + r"\surface_points_comma.csv"
    )
)
...
data.structural_frame.fault_relations = np.array([[0, 1],
                                                  [0, 0]])
