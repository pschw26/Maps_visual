...
class CreateTopo(ThreeDScene):
    ...
    def construct(self):
        ...
        surface2 = Surface(
            lambda u, v: axes.c2p(*np.array([u, v, 0.4*(u+v)])),
            u_range=[-2, 2],
            v_range=[-2, 2],
        ...
        X, Y = np.meshgrid(np.linspace(-2, 2, self.res*2), np.linspace(-2, 2, self.res*2))
        ...
        surf2 = np.array([(lambda u, v: axes.c2p(*np.array([u, v, 0.4*(u+v)])))(element[0], element[1])for element in prod])
...
#         print(outcrop_points.shape)
        ...
        labels = ['A', 'B', 'C', 'A\prime', 'B\prime', 'C\prime']
        point_group1 = []
        point_group2 = []
        lines = []
        tex_labels = []
        ...
        for i in range(3):
            ...
            line = DashedVMobject(Line(start=p, end=np.array([coords[i][0], coords[i][1], 0.4*(coords[i][0]+coords[i][1])])), num_dashes=10)
            lines.append(line)
            
            p_prime = Dot3D(point=np.array([coords[i][0], coords[i][1], 0.4*(coords[i][0]+coords[i][1])]), radius=0.04)
...
#         self.add(axes, x_label, y_label, z_label)
        ...
        self.play(Create(VGroup(*tex_labels[0::2])))
        self.wait()
        ...
        self.play(Create(VGroup(*tex_labels[1::2])))
        self.wait()
